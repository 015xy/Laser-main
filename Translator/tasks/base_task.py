...
class BaseTask:
    ...
    def train_step(self, model, samples):
        ...
        if 'loss_moe' in output:
            loss_moe = output['loss_moe']
        else:
            loss_moe = None
        
        return loss, loss_lm, loss_itc, loss_itm, loss_moe
    ...
    def _train_inner_loop(
        ...
        metric_logger.add_meter("loss_lm", SmoothedValue(window_size=1, fmt="{value:.8f}"))
        # loss, loss_lm, loss_itc, loss_itm, loss_moe
        metric_logger.add_meter("loss_itc", SmoothedValue(window_size=1, fmt="{value:.8f}"))
        metric_logger.add_meter("loss_itm", SmoothedValue(window_size=1, fmt="{value:.8f}"))
        metric_logger.add_meter("loss_moe", SmoothedValue(window_size=1, fmt="{value:.8f}"))
        ...
        for network_input in metric_logger.log_every(data_loader, log_freq, iters_per_epoch, header):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                loss, loss_lm, loss_itc, loss_itm, loss_moe = self.train_step(model=model, samples=network_input)
